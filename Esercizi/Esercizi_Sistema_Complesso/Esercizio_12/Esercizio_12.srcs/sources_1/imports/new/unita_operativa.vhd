...
architecture structural of unita_operativa is
	...
	signal Mreg: std_logic_vector(3 downto 0); --segnale temporaneao tra reg8 M e mux 21
	signal op2: std_logic_vector(3 downto 0); --segnale temporaneo di uscita dal multiplexer tra mux e adder
	signal AQ_init: std_logic_vector(7 downto 0); --segnale in input all'SR 
	signal AQ_in: std_logic_vector(7 downto 0); --segnale in input all'SR 
	signal AQ_out: std_logic_vector(7 downto 0); --segnale temporaneo uscita dell'SR
	signal temp_d: std_logic :='0';
	signal temp_F: std_logic :='0';
	signal sum: std_logic_vector(3 downto 0); --uscita del parallel adder 
...
begin
	...
	M: registro4 port map(Y, clock, reset, loadM, Mreg);
	
	-- mux per selezionare moltiplicando oppure 0 come secondo operando della somma
	MUX_molt: mux_21 generic map (width => 4) port map("0000", Mreg, selM, op2);
    ...
    AQ_init <= "0000" & X;  --valore da inserire all'inizio nello shift register
	...
	MUX_SR_parallel_in : mux_21 generic map (width => 8) port map(AQ_init, AQ_sum_in, selAQ, AQ_in);    
